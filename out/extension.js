...
// ブックマークをエクスポートする
async function exportBookmarks(provider) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
    ...
    const defaultUri = vscode.Uri.file(path.join(workspaceFolders[0].uri.fsPath, 'core-anchor-bookmarks.json'));
...
// ブックマークをインポートする
async function importBookmarks(provider) {
    ...
    if (choice.value === 'replace') {
        // 既存データを丸ごと置き換え
        saveBookmarks(importData.bookmarks);
        provider.saveBookmarksMeta(importData.bookmarksMeta);
    }
    else {
        // ── Merge ────────────────────────────────────────────────────────────
        ...
        const conflictItems = [];
        for (const [filePath, importedBMs] of Object.entries(importData.bookmarks)) {
        ...
        const mergedMeta = {
            ...
            globalSortType: existingMeta.globalSortType ??
                importData.bookmarksMeta.globalSortType,
        };
